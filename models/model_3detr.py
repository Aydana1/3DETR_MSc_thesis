...
# for Vizualization & Normalization
import time
import numpy as np
# import pandas as pd
...
class Model3DETR(nn.Module):
    ...
    def __init__(
        ...
        self.backbone_net = Pointnet2Backbone(input_feature_dim=0, width=1) # for now width=1, encoder_dim=256
        ...
        # self.linear2 = nn.Linear(dim_feedforward, d_model2, bias=ffn_use_bias)
    ...
    def run_encoder(self, point_clouds):
        ...
        fp2_xyz = end_points['fp2_xyz']
        fp2_features = end_points['fp2_features']
        fp2_inds = end_points['fp2_inds']

        fp2_features = fp2_features.permute(2, 0, 1)
        # print("fp feats: ", fp2_features.shape)
        # print("backbone: ", features.shape, xyz.shape)
        ###

        xyz, features = self._break_up_pc(point_clouds)
        pre_enc_xyz, pre_enc_features, pre_enc_inds = self.pre_encoder(xyz, features) 
        # torch.Size([8, 256, 2048]) torch.Size([8, 2048, 3])
        # print("Pre-encoder: ", pre_enc_features.shape, pre_enc_xyz.shape)
        ...
        # Method 1 (tried). Fusing with Pre-enc
        # fused_feats = fp2_features + pre_enc_features

        enc_xyz, enc_features, enc_inds = self.encoder(
            fp2_features, xyz=fp2_xyz  # fp2_features, fp2_xyz
        )
        # print("enc feats: ", enc_features.shape)

        # Method 2 (tried). Fusing with Enc (you can play around with enc_feats vs. fp_feats to the encoder)
        # fused_feats = fp2_features + enc_features
        # enc_features = fused_feats
        
        # Method 3. Fusing both
        # Method 4. Fusing with Pre-enc
        # fused_feats = fp2_features + pre_enc_features

        # Visualize feat-s
        # todo: Normalize, visualize 
        # fp2_features = fp2_features.permute(1, 0, 2)
        # enc_features = enc_features.permute(1, 0, 2)
        # # pre_enc_features = pre_enc_features.permute(1, 0, 2)

        # # X = torch.empty((enc_features.shape[1], enc_features.shape[2])) # (1024, 256)
        # X2 = torch.empty((enc_features.shape[1], enc_features.shape[2])) # ENCODER
        # # X3 = torch.empty((enc_features.shape[1], enc_features.shape[2])) # PRE-ENCODER

        # #X4 = torch.empty((enc_features.shape[1], enc_features.shape[2]))  # Normalized PRE-ENCODER 
        # norm_enc_features = torch.empty((enc_features.shape[0], enc_features.shape[1], enc_features.shape[2]))
        # # norm_fp2_features = torch.empty((enc_features.shape[0], enc_features.shape[1], enc_features.shape[2]))

        # for i in range(enc_features.shape[0]): # across batches: fused_feats.shape[0]
        #     # print("batch sample ", i)
        #     # X = fp2_features[i] # feature columns each batch has (1024, 256)
        #     X2 = enc_features[i]
        #     # X3 = pre_enc_features[i]
        #     # std_scale = preprocessing.MinMaxScaler(feature_range=(-1, 1)).fit(X.cpu().detach().numpy())
        #     std_scale2 = preprocessing.MinMaxScaler(feature_range=(0, 1)).fit(X2.cpu().detach().numpy())
        #     # norm_fp2_feats = std_scale.transform(X.cpu().detach().numpy())
        #     norm_enc_feats = std_scale2.transform(X2.cpu().detach().numpy())
        #     # X4 = norm_enc_feats
        #     norm_enc_feats = torch.from_numpy(norm_enc_feats)
        #     # norm_fp2_feats = torch.from_numpy(norm_fp2_feats)
        #     if torch.cuda.is_available():
        #         # print("CUDA IS AVAILABLE")
        #         norm_enc_feats = norm_enc_feats.cuda()
        #         # norm_fp2_feats = norm_fp2_feats.cuda()
        #     norm_enc_features[i] = norm_enc_feats #norm_enc_feats.to('cuda:0')
        #     # norm_fp2_features[i] = norm_fp2_feats

        # if torch.cuda.is_available():
        #         # print("CUDA IS AVAILABLE")
        #         norm_enc_features = norm_enc_features.cuda()
        #         # norm_fp2_features = norm_fp2_features.cuda()

        # # fused_feats = fp2_features + norm_enc_features
        # fused_feats = torch.cat((norm_enc_features, fp2_features), 2)
        # fused_feats = self.linear2(self.dropout(self.activation(self.linear1(fused_feats))))
        # # print("fused feats after NORM: ", fused_feats.shape)
        # fused_feats = fused_feats.permute(1,0,2)
        # enc_features = fused_feats

        enc_inds = enc_inds.type(torch.int64) # for 3DETR-m only
        # print("encoder features: ", enc_features.shape, enc_xyz.shape)
        if enc_inds is None:
            # encoder does not perform any downsampling
            enc_inds = fp2_inds #pre_enc_inds #
        else:
            # use gather here to ensure that it works for both FPS and random sampling
            enc_inds = torch.gather(fp2_inds, 1, enc_inds)   #pre_enc_inds
